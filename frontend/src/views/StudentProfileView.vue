...
function openEditProfileModal() {
  ...
  // اطمینان از وجود فیلدهای firstName و lastName
  // اگر از backend نیامده باشند، از فیلد name استخراج می‌کنیم
  if (!editableProfile.value.firstName && !editableProfile.value.lastName && editableProfile.value.name) {
    const nameParts = editableProfile.value.name.trim().split(' ')
    editableProfile.value.firstName = nameParts[0] || ''
    editableProfile.value.lastName = nameParts.slice(1).join(' ') || ''
  }
...
async function submitProfileEdit() {
  try {
    // ترکیب firstName و lastName به یک فیلد name برای ارسال به backend
    const profileData = {
    ...
    await api.updateStudentProfile(studentId, profileData);
...
const assignmentColumns = [
  ...
  { key: 'assessorApollonyar', label: 'شخص مصحح' },
...
const callColumns = [
  ...
  { key: 'callDate', label: 'تاریخ تماس' }, // ستون جدید
  { key: 'daysRemaining', label: 'روزهای مانده' }, // ستون جدید
        ...
        <div class="card">
          ...
          <BaseTable :columns="callColumns" :data="studentCalls" :rows-per-page="10">
            ...
            <template #cell-daysRemaining="{ item }">
              <span
                v-if="item.daysRemaining !== null"
                :class="item.daysRemaining < 0 ? 'days-past' : 'days-future'"
              >
                {{
    ...
    <BaseModal :show="isEditProfileModalOpen" @close="isEditProfileModalOpen = false">
      ...
      >
        ...
        <div class="form-group">
          ...
          <input type="text" id="profile-first-name" v-model="editableProfile.firstName" />
        </div>
        <div class="form-group">
          <label for="profile-last-name">نام خانوادگی</label>
          <input type="text" id="profile-last-name" v-model="editableProfile.lastName" />
        </div>
        <div class="form-group">
